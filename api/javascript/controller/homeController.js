		...
		async index(req, res) {
			try {
				...
				if (req.user.role === "user") {
					const menuString = await queryAllLand(req.user.userId, req.user.role)
					const menuCoString = await queryAllLandsCoUserAndAdmin(req.user.userId, req.user.role)
				...
				return res.status(200).json({ error: false, allLands })
