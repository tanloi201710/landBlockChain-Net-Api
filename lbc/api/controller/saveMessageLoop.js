...
async function saveMessageTransferLandSuccessLoop(listOld, listNew, keyLand) {
    ...
    if (typeof listOld == 'object') {
        ...
        for (let i = 0; i < listNewUserHandle.length; i++) {
            await saveMessage(listNewUserHandle[i], `Admin đã xác nhận đất có mã ${keyLand}, mãnh đất đã được chuyển thành công cho người sở hữu ${listNew.join('-')}`)
        }
    } else {
        await saveMessage(listOld, `Admin đã xác nhận đất có mã ${keyLand}, mãnh đất đã được chuyển thành công cho người sở hữu ${listNewUserHandle.join('-')}`)
